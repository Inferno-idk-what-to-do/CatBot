...
client.on("messageCreate", (message) => {
  ...
  // pause that only works for me
  if(message.content.toLowerCase().startsWith("catbot pause") && message.author.id == "517699048489091083" && acceptMessages)
  {
    acceptMessages = false;
    console.log("bot paused");
    message.channel.send("am paused -> `catbot resume` to resume");
  }
  // resume that only works for me
  else if(message.content.toLowerCase().startsWith("catbot resume") && message.author.id == "517699048489091083" && !acceptMessages)
  {
    acceptMessages = true;
  ...
  if(message.guild.id == "779787578039074857" || message.channel.id == "909670105263243324" || 
    message.channel.id == "933826202039353474" || message.channel.id == "1003166195643596811")
  {
    // status that works for anyone
    if(message.content.toLowerCase().startsWith("catbot status"))
    {
      message.channel.send("am " + (acceptMessages ? "online" : "offline"));
    }
    if(acceptMessages)
    {
      if (message.content.toLowerCase().startsWith("gibcat -u")) 
      {
        ...
        console.log("message sending with the url -> " + img);
      ...
      else if (message.content.toLowerCase().startsWith("gibcat"))
      {
        async function sameCat()
        {
          ...
          if(message.content.indexOf(" ") == -1)
          {
            ...
            console.log("message sending with the image url -> https://cataas.com/c");
          }
          else
          {
